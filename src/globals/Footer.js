      ...
      <div className="2xl:container px-8 py-16 2xl:px-0 mx-auto grid grid-cols-6 gap-4">
        <div className="col-span-6 flex flex-wrap items-center justify-center lg:justify-start">
          <Image
            ...
            src="/logo_black.svg"
            alt="Strikkestedet sort logo"
          ></Image>
          <h3 className="font-serif footer-text-center text-xl lg:text-3xl align-middle">
          ...
          {address.map((item) => {
            return (
              <AddressList
                key={item.id}
                {...item}
                color="text-black-70 footer-text-center"
              ></AddressList>
            );
          })}
        </div>
        ...
        <div className="col-span-6 lg:col-span-2 mb-4">
          ...
          <div className="flex justify-center">
            <a
              className="mx-1 transition opacity-70 hover:opacity-100"
  ...
  return (
    <li>
      <Link href={`/${props.ancestors.edges[0].node.slug}/${props.slug}`}>
        <a className="transition-all hover:text-black hover:underline">
          {props.title}
        </a>
      </Link>
    </li>
